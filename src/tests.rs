...
mod tests {
    ...
    fn test_word_frequency() {
        ...
        analyzer.remove_special_characters();
        analyzer.word_count();
        assert_eq!(analyzer.word_frequency(&"the".to_string()), 3);
        assert_eq!(analyzer.word_frequency(&"quick".to_string()), 2);
        assert_eq!(analyzer.word_frequency(&"fox".to_string()), 2);
        assert_eq!(analyzer.word_frequency(&"jumps".to_string()), 1);
        assert_eq!(analyzer.word_frequency(&"nonexistent".to_string()), 0);
    }
    #[test]
    fn test_word_frequency_french() {
        let mut analyzer = TextAnalyzer::new("programming_tips.txt", "stop_words_french.txt").unwrap();
        analyzer.content = String::from("Le petit chat est sur le tapis. Le chat est mignon.");
        analyzer.remove_special_characters();
        analyzer.word_count();
        assert_eq!(analyzer.word_frequency(&"le".to_string()), 3);
        assert_eq!(analyzer.word_frequency(&"chat".to_string()), 2);
        assert_eq!(analyzer.word_frequency(&"est".to_string()), 2);
        assert_eq!(analyzer.word_frequency(&"petit".to_string()), 1);
        assert_eq!(analyzer.word_frequency(&"inexistant".to_string()), 0);
    }
    ...
    fn test_longest_sentences_french() {
        ...
        let longest = analyzer.longest_sentences();
